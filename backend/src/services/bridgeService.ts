...
export class BridgeService {
    ...
    static async getUsdcBalance(address: string, provider?: ethers.JsonRpcProvider): Promise<bigint> {
        const p = provider ?? this.getProvider();
        const usdc = new ethers.Contract(ARBITRUM_USDC, ERC20_ABI, p) as any;
        return await usdc.balanceOf(address);
    }

    static async getEthBalance(address: string, provider?: ethers.JsonRpcProvider): Promise<bigint> {
        const p = provider ?? this.getProvider();
    ...
    static async ensureAllowance(
        ...
        const usdc = new ethers.Contract(ARBITRUM_USDC, ERC20_ABI, wallet) as any;
    ...
    static async depositToHyperliquid(
        wallet: ethers.Wallet,
        amount: bigint
    ): Promise<{ txHash: string; methodUsed: string }> {
        const bridge = new ethers.Contract(HYPERLIQUID_ARBITRUM_BRIDGE, HYPERLIQUID_BRIDGE_ABI, wallet) as any;
        const methods = ['bridgeUsdc', 'deposit', 'depositUSDC'] as const;
        ...
        throw new Error(
            `Failed to deposit via bridge contract. Last error: ${
    ...
    static async sweepAgentToHyperliquid(params: {
        ...
        if (ethBalance < minEth) {
            return {
                ...
                ethBalance,
                skipped: true,
                error: `TRAPPED_FUNDS: ${params.agentAddress} has ${ethers.formatEther(
                    ethBalance
                )} ETH on Arbitrum. Send at least ${ethers.formatEther(minEth)} ETH.`,
            };
        }
