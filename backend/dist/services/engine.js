...
export class TradingEngine {
    ...
    isRunning = false;
    infoClient;
    FUNDING_THRESHOLD = 0.00001; // 0.001% (expressed as 0.00001)
    MIN_BALANCE = 5; // 5 USDC minimum
    LOOP_INTERVAL = 60000; // 60 seconds
    constructor() {
     ...
     */
    async executeAgentStrategy(agent, fundingRate) {
        try {
            ...
            const hasOpenPositions = positions.assetPositions?.some((p) => parseFloat(p.position.szi) !== 0);
            if (hasOpenPositions) {
     ...
     */
    async executeConservativeStrategy(exchangeClient, balance, fundingRate, leverage) {
     ...
     */
    async executeAggressiveStrategy(exchangeClient, balance, leverage) {
