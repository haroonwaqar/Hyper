...
async function run() {
    ...
    const minEthForGasWei = minEth && !Number.isNaN(Number(minEth)) ? BigInt(Math.floor(Number(minEth) * 1e18)) : undefined;
    let agents;
    if (agentAddress) {
    ...
    for (const agent of agents) {
        try {
            const result = await BridgeService.sweepAgentToHyperliquid({
                ...
                ...(minEthForGasWei ? { minEthForGasWei } : {}),
            });
            if (result.skipped) {
                console.log(`⏭️  ${result.agentAddress} skipped. USDC=${result.usdcBalance} ETH=${result.ethBalance}${result.error ? ` | ${result.error}` : ''}`);
                continue;
            }
            console.log(`✅ ${result.agentAddress} swept.`);
            if (result.approveTxHash)
                console.log(`   Approve: ${result.approveTxHash}`);
            if (result.depositTxHash)
                console.log(`   Deposit: ${result.depositTxHash} (${result.methodUsed})`);
        }
        catch (error) {
...
//# sourceMappingURL=sweep-to-hyperliquid.js.map